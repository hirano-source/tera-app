...
const S = {
  ...
  createTask: {
    ...
    properties: {
      ...
      parentTaskId: { type: 'string', description: '親タスクのid（指定で入れ子。大→中→小→サブと何段でもブレイクダウン可）' },
      size: { type: 'string', enum: ['big', 'mid', 'small', 'sub'], description: '粒度（工数の大きさ）big≒3ヶ月/mid≒1週間/small≒3日/sub≒3時間。緊急度(priority)とは別軸。入れ子で作るときは親より1段小さくする' },
  ...
  update: {
    ...
    properties: {
      ...
      title: { type: 'string' },
      size: { type: 'string', enum: ['big', 'mid', 'small', 'sub'], description: '粒度 big≒3ヶ月/mid≒1週間/small≒3日/sub≒3時間' },

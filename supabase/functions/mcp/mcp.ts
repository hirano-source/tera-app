...
const S = {
  ...
  update: {
    ...
    properties: {
      ...
      title: { type: 'string' },
      goalId: { type: 'string', description: '【移動】別のゴールの直下へ付け替える。あわせて parentTaskId を空にするとそのゴール直下のタスクになる' },
      parentTaskId: { type: 'string', description: '【移動】別のタスクの下（サブタスク）へ付け替える。循環（自分やその子孫の下）にしないこと' },
  ...
  updateGoal: {
    ...
    properties: {
      ...
      parentId: { type: 'string', description: '【移動】別の大目標／ゴールの下へ付け替える。最上位にするなら null（大目標自体の付け替えはしない＝大目標は事業設定で扱う）' },
    },
    required: ['goalId'],
  },
...
const TOOLS: Record<string, Tool> = {
  ...
  update_goal: {
    description: 'ゴールの中身を更新する。理想の状態(idealState)/現状(current)/その差(gap)/完了の基準(criteria)/期日/担当/進捗/タイトル。parentId で別の大目標・ゴールの下へ移動（付け替え）もできる。owner/adminのみ。',
  ...
  update_task: {
    description: 'タスクを更新する。状態(todo/doing/done/blocked)・タイトルのほか、priority/完了基準/やり方/期限/recurrence、詰まり時は blockerType/blockerOwner/blockerNote も指定できる。goalId/parentTaskId で別のゴール直下や別タスクの下へ移動（付け替え）もできる。',
